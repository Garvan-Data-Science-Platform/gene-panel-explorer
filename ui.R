...
versionDate <- "2024-Nov-15" # format(as.Date(Sys.Date(), format="%Y-%m-%d"), "%d/%m/%Y")
workingName <- "Gene Panel Explorer"

panel_date_start <- "2023-12-01"
  ...
  mainPanel(
    ...
    tabsetPanel(
      ...
      tabPanel(
        ...
        conditionalPanel(
          condition = "input.showPanelInputHelp % 2 == 1",
        ...
        fluidRow(
          ...
          conditionalPanel(
            condition = "input.disease.length > 0",
            column(
              3,
              checkboxGroupInput(
                inputId = "selectedPanels",
          ...
          conditionalPanel(
            ...
            column(
              3,
              checkboxGroupInput(
                inputId = "selectedPanelsExperimental",
      ...
      tabPanel(
        ...
        conditionalPanel(
          condition = "input.showViewInputHelp % 2 == 1",
          ...
          h5("DisGeNET evidence level values can be (in decreasing order) 'definitive', 'strong', 'moderate', 'limited' or 'NA'. See ", tags$a(
            href = "https://www.disgenet.org/dbinfo#score:~:text=Evidence%20Level,as%20limited",
            target = "_blank",
            "here"
          ), " for how they're calculated."),
          h5("PanelApp Australia / Genomics England evidence level values can be (in decreasing order) 'strong', 'moderate', 'limited' or 'NA'. See", tags$a(
            href = "https://panelapp.genomicsengland.co.uk/#!Guidelines",
            target = "_blank",
            "here"
          ), " for how they're calculated.")
        ),
        ...
        fluidRow(
          conditionalPanel(
            condition = 'input.inputPlots.includes("UpSet Plot")',
            column(
              6,
              h3("UpSet Plot"),
              conditionalPanel(
                condition = "input.selectedPanels.length + (input.addGenes.length > 0) + input.selectedPanelsCurated.length + input.selectedPanelsExperimental.length < 2",
          ...
          conditionalPanel(
            condition = 'input.inputPlots.includes("Venn Diagram")',
            column(
              6,
              h3("Venn Diagram"),
              conditionalPanel(
          ...
          conditionalPanel(
            condition = 'input.inputPlots.includes("Bar Plot")',
            column(
              6,
              h3("Bar Plot"),
              conditionalPanel(
                condition = "input.selectedPanels.length + (input.addGenes.length > 0) + input.selectedPanelsCurated.length + input.selectedPanelsExperimental.length == 0",
      ...
      tabPanel(
        ...
        conditionalPanel(
          condition = "input.showSTRINGHelp % 2 == 1",
          ...
          h5(tags$a(
            href = "http://version10.string-db.org/help/faq/#how-are-the-scores-computed",
            target = "_blank",
            "How are STRING protein-protein interaction scores calculated?"
          )),
          h5(tags$a(
            href = "https://string-db.org/cgi/info?sessionId=bWfA0C3yBWIJ&footer_active_subpage=scores",
            target = "_blank",
            "What do STRING protein-protein interaction scores represent?"
          )),
          h5(tags$a(
            href = "http://version10.string-db.org/help/faq/#:~:text=escore%20%2D%20experimental%20score,names%20in%20abstracts).",
            target = "_blank",
      ...
      tabPanel(
        ...
        conditionalPanel(
          condition = "input.showGTExGHelp % 2 == 1",
          ...
          tags$br(),
          h5(tags$a(
            href = "https://www.datanovia.com/en/lessons/determining-the-optimal-number-of-clusters-3-must-know-methods/#gap-statistic-method",
            target = "_blank",
        ...
        conditionalPanel(
          condition = "input.showAdvTissue % 2 == 1",
          tags$br(),
          fluidRow(
        ...
        tags$hr(),
        conditionalPanel(
          condition = "input.showIndividualGTEx % 2 == 1",
        ...
        tags$hr(),
        conditionalPanel(
          condition = 'input.gtexClusterPlots.includes("Gap Statistic")',
          plotOutput("gtexGapStatistic", width = "100%", height = "auto")
        ),
        conditionalPanel(
          condition = 'input.gtexClusterPlots.includes("Heatmap")',
          plotOutput("gtexHeatmap", width = "100%", height = "auto")
        ),
        conditionalPanel(
          condition = 'input.gtexClusterPlots.includes("Dendrogram")',
          plotOutput("gtexDendrogram"),
        ),
        conditionalPanel(
          condition = 'input.gtexClusterPlots.includes("Table")',
          DTOutput("gtexClusters")
        ),
        tags$hr(),
        ...
        tags$hr(),
        conditionalPanel(
          condition = 'input.gtexResultPlots.includes("Heatmap")',
          column(
            6,
        ...
        conditionalPanel(
          condition = 'input.gtexResultPlots.includes("Radar")',
          column(
            6,
        ...
        conditionalPanel(
          condition = 'input.gtexResultPlots.includes("Heatmap") | input.gtexResultPlots.includes("Radar")',
          tags$hr()
        ),
        conditionalPanel(
          condition = 'input.gtexResultPlots.includes("Table")',
          DTOutput("gtextable")
        ),
        tags$hr(),
        fluidRow(
          ...
          column(
            6,
            textAreaInput(
              inputId = "gtexProgressiveTextOutput",
      ...
      tabPanel(
        ...
        fluidRow( # dropdown to select which HPO phenotype to take as results
          column(4, selectizeInput(
            inputId = "hpoResultDropdown",
            choices = NULL, # will be added once results are found
            label = "HPO phenotype-to-gene list:"
          )),
          column(
            8, # Add a text output area, to copy out space-separated result genes
            textAreaInput(
        ...
        fluidRow(
          textAreaInput(
            inputId = "hpoTextOutputRecurringGenes",
      ...
      tabPanel(
        ...
        conditionalPanel(
          condition = "input.showGOHelp % 2 == 1",
          h5(tags$a(
            href = "https://geneontology.org/docs/go-annotations/#annotation-qualifiers",
            target = "_blank",
            "Intro to GO annotations"
          )),
          h5(tags$a(
            href = "https://link.springer.com/protocol/10.1007/978-1-4939-3743-1_14#:~:text=Modification%20of%20Annotation%20Meaning%20by%20Qualifiers",
            target = "_blank",
        ...
        tags$hr(),
        withSpinner(DTOutput("GOannotations"),
          type = 2
        ),
      ...
      tabPanel(
        ...
        conditionalPanel(
          condition = "input.validationDisplay.includes('ClinVar gene-disease associations')",
          ...
          tags$br(),
          tags$br(),
          DTOutput("clinvarInfo")
        ),
        conditionalPanel(
          condition = "input.validationDisplay.includes('ClinVar gene variant information')",
          ...
          fluidRow(
            ...
            column(
              3,
              checkboxGroupInput(
                inputId = "variantClinicalSignificance",
        ...
        conditionalPanel(
          condition = "input.validationDisplay.includes('Check gene panels in local data')",
