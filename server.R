...
hpo <- read_tsv(file = "Data/HPO/genes_to_disease.txt", show_col_types = FALSE)
...
hpo_p2g <- read_delim("Data/HPO/phenotype_to_genes.txt",
  delim = "\t", escape_double = FALSE,
...
stringIndex <- read.csv("Data/STRING/9606.protein.info.v12.0.txt",
  header = TRUE,
  encoding = "utf-8", quote = "", sep = "\t"
)
colnames(stringIndex) <- c(
...
function(input, output, session) {
  ...
  observeEvent(input$addPanel, {
    ...
    showModal(modalDialog(
      ...
      conditionalPanel(
        condition = 'input.addPanelGenesMethod == "Text block"',
      ...
      conditionalPanel(
        condition = 'input.addPanelGenesMethod == "Source & url"',
      ...
      radioButtons(
        inputId = "addPanelProtected",
        label = "Protected (cannot be deleted)?",
        choices = c("Yes", "No"),
        selected = "No"
      ),
      dateInput(
        inputId = "addPanelDate",
        label = "Panel Date:",
        value = Sys.Date()
      ),
  ...
  observeEvent(input$editPanel, {
    ...
    showModal(modalDialog(
      ...
      # display panel info
      conditionalPanel(
        condition = "input.editPanelName.length > 0",
        # panel genes
        # add / remove
        textAreaInput(
          inputId = "editPanelPreview",
          label = "Panel Genes:",
          resize = "vertical"
        ),
        # Panel date
        dateInput(
          inputId = "editPanelDate",
          label = "Panel Date:"
        ),
        # panel type
        radioButtons(
          inputId = "editPanelType",
          label = "Panel Type:",
          choices = c(
            "Commercial",
            "Curated",
  ...
  observeEvent(input$removePanel, {
    ...
    showModal(modalDialog(
      ...
      conditionalPanel(
        condition = 'input.removePanelName != ""',
        textAreaInput(
          inputId = "previewRemovePanel",
  ...
  output$STRINGerrormsg <- renderText({
    STRINGerrormsg()
  })
  ...
  observeEvent(input$gtexClusterStart, {
    ...
    output$gtexClusters <- renderDT({
      clusters_df
    })

    updateRadioButtons(
      inputId = "chooseCluster",
  ...
  observeEvent(input$gtexStart, {
    ...
    # updatetextareainput
    updateTextAreaInput("gtexTextOutput",
      session = session,
    ...
    output$gtexRadar <- renderPlot(
      {
        ...
        ggradar(
          plot.data = radarData, group.line.width = 1,
    ...
    # output table
    output$gtextable <- renderDT(gtexTable_filtered)
  })
  ...
  output$individualGTExTable <- renderDT({
  ...
  output$comboPlot <- renderPlot({
    ...
    ggplot(result, aes(x = Correlation, y = score2, label = label)) +
      geom_point(alpha = 0.25, size = (1 + result$sdbScore / 100)) +
      xlim(c(-1, 1)) +
      geom_label_repel(
        box.padding = 0.4, max.overlaps = Inf, size = 3,
        segment.size = 0.25, segment.color = "#636363"
      ) +
      xlab("GTEx spatial correlation") +
      ylab("Combined score (GTEx & STRING)") +
      ggtitle(paste0("Input: ", input$gene))
  })

  # unused
  output$rankTable <- renderDT({ # Generate rank result table
  })
  ...
  observeEvent(input$hpoMakePlots, {
    ...
    output$hpoPlot <- renderPlot({
      ...
      updateSelectizeInput(
        inputId = "hpoResultDropdown", server = TRUE,
      ...
      hpoPlot2 <- ggplot(overlapTable[idx, ], aes(y = overlap, x = overlapPercent)) +
        ...
        geom_text_repel(aes(label = phenotypes),
          max.overlaps = Inf, size = 2.5,
          min.segment.length = 0, box.padding = 1, segment.alpha = 0.2
        )
      hpoPlot3 <- ggplot(
        topHpoGenes[1:min(20, length(topHpoGenes$gene)), ],
        aes(y = reorder(gene, counts), x = counts, fill = inInput)
      ) +
        geom_col(width = 0.7) +
        labs(fill = "Input panel gene") +
        ylab("Gene") +
        xlab("# inclusions in top Phenotypes") +
        theme_light()
  ...
  output$overlapVenn <- renderPlot({
    data <- list(
      "Set 1" = textblock2genes(input$countOverlap1, input$useOMIM == "Yes"),
  ...
  output$mostCommonAnnotations <- renderPlot({
    ...
    df <- data.frame(
      annotations = unique_annotations$Plaintext,
      counts = counts,
      Type = aspects
    )
    df <- df[order(-counts), ]
  ...
  output$clinvarInfo <- renderDT({
    ...
    output$clinvarTable <- downloadHandler(
      filename = function() {
        "clinvar_gene_disease_associations.csv"
      },
      content = function(file) {
  ...
  output$validationLocalTable <- renderDT({
